...
class DecodeAndMigrateTests extends FlatSpec with Matchers {
  ...
  "DecodeAndMigrate" should "choose the latest decoder" in {
    ...
    implicit val y = DecodeAndMigrateBuilder.base[User, A, _1, UserV1, UserV2](producerOfAs)
  ...
  it should "choose latest and then migrate" in {
    ...
    implicit val y = DecodeAndMigrateBuilder.base[User, A, _2, UserV2, UserV3](producerOfAs)(
      implicitly[Decoder[A, UserV2]],
      implicitly[Decoder[A, UserV3]],
      implicitly[MigrationBuilder.Aux[User, _2, _3, UserV2, UserV3]])

    implicit val y1 = DecodeAndMigrateBuilder.decodeOrRecurse[User, A, _1, _3, UserV1, UserV3](producerOfAs)
