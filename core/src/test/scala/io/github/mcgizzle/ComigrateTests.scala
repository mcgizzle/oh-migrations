...
class ComigrateTests extends FlatSpec with Matchers {
  ...
  it should "work for a long chain" in {
      ...
      implicit val V5toV6: UserV6 +=> UserV5 =
        MigrationFunction(a => UserV5(Name(a.firstName.value + " " + a.lastName.value), a.age, None, None))
      implicit val V6toV7: UserV7 +=> UserV6 =
        MigrationFunction(a => UserV6(a.firstName, a.lastName, a.age, a.email, a.address.value))
      ...
      Comigrate[User_]
        .from[_8, _1]
        .apply(UserV8(FirstName("John"), LastName("Smith"), Age(43), ContactInfo(None, Address(None)))) shouldBe UserV1(
        "John Smith",
        43
      )
    }
  ...
  it should "not work for wrong tag" in {
      ...
      illTyped(
        """io.github.mcgizzle.Comigrate[Dog].from[_2, _0].apply(Adam("hi")) shouldBe Bob("hi")""",
  ...
  it should "fail going forward a version" in {
      ...
      illTyped(
        """io.github.mcgizzle.Comigrate[User].from[_1, _2].apply(UserV2(Name(""), 4))""",
        "could not find implicit value for parameter m.*"
      )
    }
