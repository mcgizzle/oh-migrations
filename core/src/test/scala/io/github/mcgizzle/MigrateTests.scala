...
class MigrateTests extends FlatSpec with Matchers {
  ...
  it should "work for a long chain" in {
      ...
      implicit val V6toV7: UserV6 +=> UserV7 =
        MigrationFunction(a => UserV7(a.firstName, a.lastName, a.age, a.email, Address(a.address)))
      implicit val V7toV8: UserV7 +=> UserV8 =
        MigrationFunction(a => UserV8(a.firstName, a.lastName, a.age, ContactInfo(a.email, a.address)))

      Migrate[User_].from[_1, _8].apply(UserV1("John Smith", 43)) shouldBe UserV8(
        FirstName("John"),
        LastName("Smith"),
        Age(43),
        ContactInfo(None, Address(None))
      )
    }
  ...
  it should "not work for wrong tag" in {
      ...
      illTyped(
        """io.github.mcgizzle.Migrate[Dog].from[_0, _2].apply(Adam("hi")) shouldBe Bob("hi")""",
        "could not find implicit value for parameter m.*"
      )
  ...
  it should "fail going back a version" in {
      ...
      illTyped(
        """io.github.mcgizzle.Migrate[User].from[_2, _1].apply(UserV2(Name(""), 4))""",
        "could not find implicit value for parameter m.*"
      )
    }
